...
use App\Http\Requests\Site\GuestCheckoutAddressRequest;
use App\Http\Requests\Site\GuestCheckoutOrderReviewRequest;
use App\Option;
use App\Order;
use App\OrderItem;
use App\Payment;
...
class GuestCheckoutController extends Controller
{
    ...
    public function rsr_product_count()
    {
        ...
        // return $rsrProductCount;
        return $rsrProductCount > 0 ? true : false;
    }

    // public function is_product_available_to_state()
    // {
    //     $cartItems = Cart::instance('cart')->content();
    ...
    //                 Product::where('status', 1)->where('slug', $item->options['slug'])->first()->$stateSessionData == 'Y';
    //                 $cartStates[] = 0;
    //             }else{
    //                 RsrProduct::where('rsr_stock_number', $item->options['slug'])->first()->$stateSessionData == 'Y';
    //                 $cartStates[] = 1;
    //             }
    //         }
    //     }
    ...
    public function get_guest_checkout_verification()
    {
        ...
        return view('site.guest-checkout.verification');
    }
    ...
    public function get_guest_checkout_state_verification()
    {
        ...
        if (!session()->has('stateSession')) {
            return redirect('address')->with('error', 'Please, Enter your address');
        }
        ...
        return view('site.guest-checkout.state_verification', compact('stateSessionData'));
    }
    ...
    public function get_guest_ffl_dealer()
    {
        ...
        return view('site.guest-checkout.ffl_dealer', compact('fflDealers'));
    }
    ...
    // 1
    public function get_guest_checkout_address()
    {
        if (Cart::instance('cart')->count() == 0) {
            abort(404);
        }
        ...
        if(Cart::instance('cart')->content()->where('options.is_age_verification_required', 1)->count() > 0){
            if (!session()->has('ageVerificationSession')) {
                return redirect('verification')->with('error', 'Please, Verify your age.');
            }
        }

        // $countries = $this->country->pluck('nicename', 'id');
        $countries = $this->country->where('id', 226)->pluck('nicename', 'id');
        return view('site.guest-checkout.address', compact('countries'));
    }

    public function post_guest_checkout_address(GuestCheckoutAddressRequest $request)
    {
        // Data for shipping
        ...
        try {
            ...
            if ($request->data_from == "checkout") {
                ...
                return redirect('state-verification');
            }
    ...
    // 2
    public function get_guest_checkout_shipping()
    {
        if (Cart::instance('cart')->count() == 0) {
            abort(404);
        }
        ...
        // $shippingData = $this->shipping->where('status', 1)->whereNotIn('id', [2, 4, 5])->get();


        // UPS Shipping method
        \Log::info($this->get_rating());
        ...
        \Log::info($this->get_fedex_rating());
        if(is_array($this->get_fedex_rating()) ){
            $fedExRating = $this->get_fedex_rating();
        }else{
            $fedExRating = [];
        }
        \Log::info($fedExRating);

        $shippingServices = array_merge($rating, $fedExRating);
        \Log::info( $shippingServices );

        // dd($shippingServices);
        // dump(collect($shippingServices)->where('shipping_code', '03')->first()['amount'] ?? 7.5);
        // dump(collect($shippingServices)->where('shipping_code', '02')->first()['amount'] ?? 15);
        // dump(collect($shippingServices)->where('shipping_code', 'FEDEX_GROUND')->first()['amount'] ?? 7.5);
        // dump(collect($shippingServices)->where('shipping_code', 'FEDEX_2_DAY')->first()['amount'] ?? 15);

        // $upsGround = collect($shippingServices)->where('shipping_code', '03')->first()['amount'] ?? 7.5;
        // $upsSecondDayAir = collect($shippingServices)->where('shipping_code', '02')->first()['amount'] ?? 15;
        // $fedExGround = collect($shippingServices)->where('shipping_code', 'FEDEX_GROUND')->first()['amount'] ?? 7.5;
        // $fedExSecondDayAir = collect($shippingServices)->where('shipping_code', 'FEDEX_2_DAY')->first()['amount'] ?? 15;

        // $shippingCharges = [
        //     'upsGround' => (float) $upsGround,
        //     'upsSecondDayAir' => (float) $upsSecondDayAir,
        //     'fedExGroundfedExGround' => (float) $fedExGround,
        //     'fedExSecondDayAir' => (float) $fedExSecondDayAir,
        // ];
        // dd($shippingCharges);

        return view('site.guest-checkout.shipping', compact('shippingData', 'rating', 'fedExRating', 'shippingServices'));

        // return view('site.guest-checkout.shipping', compact('shippingData'));
    }

    public function post_guest_checkout_shipping(Request $request)
    {
        // dd($request->all());
    ...
    // 3
    public function get_guest_checkout_payment()
    {
        if (Cart::instance('cart')->count() == 0) {
            abort(404);
        }
        ...
        if (!session()->has('shippingSingleDataSession')) {
            return redirect('shipping-methods')->with('error', 'Please, Select a shipping method.');
        }

        $payments = $this->payment->where('status', 1)->get();
        return view('site.guest-checkout.payment', compact('payments'));
    }

    public function post_guest_checkout_payment(Request $request)
    {
    ...
    // 4
    public function get_guest_checkout_order_review()
    {
        if (Cart::instance('cart')->count() == 0) {
            abort(404);
        }
        ...
        if (!session()->has('paymentSingleDataSession')) {
            return redirect('payment-methods')->with('error', 'Please, Select a payment method.');
        }
        ...
        // FedEx Shipping method
        \Log::debug($this->get_fedex_rating());
        if(is_array($this->get_fedex_rating()) ){
            $fedExRating = $this->get_fedex_rating();
        }else{
            $fedExRating = [];
        }
        \Log::debug($fedExRating);

        // Total Shipping services
        $shippingServices = array_merge($rating, $fedExRating);
        \Log::debug($shippingServices);

        return view('site.guest-checkout.review_style_1', compact('cartItems', 'subTotal', 'shippingAmount', 'grandTotal', 'mainShippingMethod', 'shippingServiceName', 'paymentMethod', 'countries', 'shippingData', 'payments', 'rating', 'fedExRating', 'shippingServices', 'totalRsrShippingAMount'));
    }

    public function post_guest_checkout_order_review(GuestCheckoutOrderReviewRequest $request)
    {
        if (Cart::instance('cart')->count() == 0) {
        ...
        try {
            ...
            if($isSameAsBilling == 1){
                $this->customer->delivery_address = $customerData['billing_address'];
                $this->customer->delivery_city = $customerData['billing_city'];
                $this->customer->delivery_state = $customerData['billing_state'];
                $this->customer->delivery_postal_code = $customerData['billing_postal_code'];
        ...
        try {
            ...
            if (!empty($rateReply->RateReplyDetails)) {
                ...
                foreach ($rateReply->Notifications as $Notification) {
                    ...
                    return redirect()->back()->with('error', 'FedEx - '.$Notification->Message);
                }
            }
    ...
    public function get_order_completed()
    {
        ...
        return view('site.payment.completed', compact('orderNo'));
    }
    ...
    // ---- Authorize.net -----
    public function get_authorize_net()
    {
        $authorizeNetToken = $this->getAnAcceptPaymentPage();
        return view('site.payment.authorize_net', compact('authorizeNetToken'));
    }
    ...
    public function get_authorize_net_payment_status(Request $request)
    {
        $this->order
            ->where('id', Session('order')['id'])
            ->update([
    ...
    function getAnAcceptPaymentPage()
    {
        ...
        $setting3->setSettingValue(
            "{\"url\": \"http://localhost/boomstickdefense.dev/public/authorize-net-payment-status\", \"cancelUrl\": \"http://localhost/boomstickdefense.dev/public\", \"showReceipt\": true}"
        );
        ...
        if (($response != null) && ($response->getMessages()->getResultCode() == "Ok")) {
            ...
            return redirect()->back()->with('error', 'ERROR :  Failed to get hosted payment page token');
        }
        // return $response;
    }

    function createAnAcceptPaymentTransaction($amount = 50)
    {
        /* Create a merchantAuthenticationType object with authentication details
           retrieved from the constants file */
        $merchantAuthentication = new AnetAPI\MerchantAuthenticationType();
