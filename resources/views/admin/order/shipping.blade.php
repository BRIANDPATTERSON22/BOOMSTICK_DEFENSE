@extends('admin.layouts.app')

@section('htmlheader_title')
    Shipping Methods
@endsection

@section('contentheader_title')
    <span class="text-capitalize"> Shipping Methods</span>
@endsection

@section('contentheader_description')
    Manage shipping methods
@endsection

@section('breadcrumb_title')
    <ol class="breadcrumb">
        <li class="text-capitalize"><a href="{{url('admin/dashboard')}}"><i class="fa fa-dashboard"></i> Dashboard</a></li>
        <li class="text-capitalize"><a href="{{url('admin/'.$module.'s')}}">{{$module}}(s)</a></li>
        <li class="text-capitalize active"> Shipping Methods</li>
    </ol>
@endsection

@section('main-content')
    @include('admin.partials.flash_message')
    
    <div class="nav-tabs-custom">
        <ul class="nav nav-tabs">
            <li class="active">
                <a href="#tab_1" data-toggle="tab"><i class="fa fa-list"></i>
                    <span>
                        @if($singleData->id) Edit Shipping Method ID: {{$singleData->id}} @else  Add New Shipping Method @endif
                    </span>
                 </a>
            </li>
            <li class="pull-right">
                <a href="{{url('admin/shippings')}}"><i class="fa fa-plus"></i> <span>Add</span></a>
            </li>
        </ul>
        <div class="tab-content">
            <div class="tab-pane active">
                <div class="row">
                    <div class="col-md-12">
                        {!! Form::model($singleData, array('files' => true, 'autocomplete' => 'off')) !!}
                        {!!csrf_field()!!}
                        {{-- <div class="box-header">
                            <h3 class="box-title">
                                @if($singleData->id) Edit Shipping Method ID: {{$singleData->id}} @else  Add New Shipping Method @endif
                            </h3>
                        </div> --}}
                        <div class="box-body">
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                                        {!! Form::label("Title *") !!}
                                        {!! Form::text('title', null, ['class' => 'form-control', 'placeholder' => 'Enter title']) !!}
                                        <em class="error-msg">{!!$errors->first('title')!!}</em>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group {{ $errors->has('time') ? 'has-error' : '' }}">
                                        {!! Form::label("Delivery Time") !!}
                                        {!! Form::text('time', null, ['class' => 'form-control', 'placeholder' => 'Enter delivery time']) !!}
                                        <em class="error-msg">{!!$errors->first('time')!!}</em>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group {{ $errors->has('amount') ? 'has-error' : '' }}">
                                        {!! Form::label("Amount") !!}
                                        {!! Form::text('amount', $singleData->amount ? $singleData->amount : 0, ['class' => 'form-control', 'placeholder' => 'Enter the amount']) !!}
                                        <em class="error-msg">{!!$errors->first('amount')!!}</em>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-sm-12 form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                                    {!! Form::label("Description") !!}
                                    {!! Form::textarea('description', null, ['class' => 'form-control', 'placeholder' => 'Enter description', 'rows' => '2']) !!}
                                    <em class="error-msg">{!!$errors->first('description')!!}</em>
                                </div>
                            </div>
                        </div>
                        <div class="box-footer">
                            @if($singleData->id)
                                <div class="pull-right form-group">
                                    <label class="switch" title="@if($singleData->status == 1) Enabled @else Disabled @endif">
                                        <input type="checkbox" name="status" value="1" @if($singleData->status == 1) checked @endif >
                                        <div class="slider round"></div>
                                    </label>
                                </div>
                            @endif
                            <div class="form-group">
                                <button type="submit" class="btn btn-success">
                                    <i class="fa fa-check-circle-o"></i> @if($singleData->id) Update @else Create @endif
                                </button>
                            </div>
                        </div>
                        {!! Form::close() !!}
                    </div>
                </div>



            </div>
        </div>
    </div>


    <div class="nav-tabs-custom">
        <ul class="nav nav-tabs">
            <li class="active"><a href="#tab_1" data-toggle="tab"><i class="fa fa-list"></i> <span>List of Shipping Methods</span></a></li>
            {{-- <li class="pull-right">
                <a href="{{url('admin/shippings')}}"><i class="fa fa-plus"></i> <span>Add</span></a>
            </li> --}}
        </ul>
        <div class="tab-content">
            <div class="tab-pane active" id="tab_1">
                <div class="row">
                    <div class="col-md-12">
                        <div class="box-body">
                            <div class="row">
                                <div class="col-md-12">
                                   {{--  <div class="box-header">
                                        <small class="pull-right">
                                            <a href="{{url('admin/shippings')}}" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Add</a>
                                        </small>
                                    </div> --}}
                                    <div class="box-body">
                                        <div class="table-responsive">
                                            <table id="dataTable" class="table table-hover">
                                                <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th>Title</th>
                                                    <th>Time</th>
                                                    <th>Amount</th>
                                                    <th>Description</th>
                                                    <th>Action</th>
                                                </tr>
                                                </thead>
                                                <tbody>
                                                <?php $count = 0; ?>
                                                @foreach ($allData as $row)
                                                    <?php $count++; ?>
                                                    <tr class="@if($row->status==0) disabledBg @endif">
                                                        <td>{{$count}}</td>
                                                        <td>{{ $row->title }}</td>
                                                        <td>{{ $row->time ? $row->time : '--' }}</td>
                                                        <td>{{ $row->amount ? $row->amount : '--'}}</td>
                                                        <td>{{ $row->description ? $row->description : '--' }}</td>
                                                        <td  style="white-space: nowrap;">
                                                            <a href="{{ url('admin/shipping/'.$row->id.'/edit')}}" class="btn btn-sm btn-warning"> <i class="fa fa-edit"> </i> Edit</a>
                                                            <a href="{{ url('admin/shipping/'.$row->id.'/delete')}}" onclick="if(!confirm('Are you sure to delete this data?')){return false;}" class="btn btn-sm btn-danger disabled"><i class="fa fa-trash-o"> </i> Delete</a>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                                </tbody>
                                            </table>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection

@section('page-script')
{{--     <script>
        $(document).ready(function () {
            $('#chkUSCitizen').removeAttr('checked');
            $('#divSSN').hide();
            $('#uk-checkbox').change(function () {
                if (this.checked) {
                    $('#uk-shipping').show();
                }
                else {
                    $('#uk-shipping').hide();
                }
            });
        });
    </script> --}}

{{--     <script type="text/javascript">
        function ukCheckbox()
        {
            if($('#uk-checkbox').is(":checked"))   
                $(".uk-shipping").show();
            else
                $(".uk-shipping").hide();
        }
    </script> --}}

 {{--    <script>
        $(document).ready(function() { ... });
        $(function () {
            $("#uk-checkbox").click(function () {
                if ($(this).is(":checked")) {
                    $("#uk-shipping").show();
                } else {
                    $("#duk-shipping").hide();
                }
            });
        });
    </script> --}}

    <script>
       /* $(document).ready(function() {
            if ($('#uk-checkbox, #europe-checkbox, #global-checkbox ').is(':checked')) {
                $('#uk-shipping').show();
                $('#europe-shipping').show();
                $('#global-shipping').show();
            } else {
                $('#uk-shipping').hide();
                $('#europe-shipping').hide();
                $('#global-shipping').hide();
            }
        });*/
        function ukCheckbox() {
          if($('#uk-checkbox').is(":checked")){
            $("#uk-shipping").show();
          }else{
            $("#uk-shipping").hide();
          }
        }
        function europeCheckbox() {
            if($('#europe-checkbox').is(":checked")){
                $("#europe-shipping").show();
            }else{
                $("#europe-shipping").hide();
            }
        }
        function globalCheckbox() {
            if($('#global-checkbox').is(":checked")){
                $("#global-shipping").show();
            }else{
                $("#global-shipping").hide();
            }
        }
    </script>

{{--     <script>
        $(document).ready(function(){
            $('input[name="is_checked_uk"]').click(function(){
                if($(this).prop("checked") == true){
                    $("#uk-shipping").show();
                    alert("Checkbox is checked.");
                }
                else if($(this).prop("checked") == false){
                    alert("Checkbox is unchecked.");
                }
            });
        });
    </script> --}}

{{--     <script>
        $(document).ready(function() {
            if ($('#uk-checkbox').is(':checked')) {
                $('#uk-shipping').show();
            } else {
                $('#uk-shipping').hide();
            }
        });
    </script> --}}
@endsection