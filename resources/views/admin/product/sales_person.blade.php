...
@section('main-content')
    <div class="nav-tabs-custom">
        ...
        <ul class="nav nav-tabs">
            <li @if(Request::is('admin/sales-person')) class="active" @endif>
                <a href="{{url('admin/sales-person')}}"><i class="fa fa-list"></i> 
                    <span>
            ...
            @yield('actions')
            <li class="pull-right">
                <a href="{{url('admin/sales-person')}}"><i class="fa fa-plus"></i> <span>Add</span></a>
            </li>
        </ul>
                        ...
                        <div class="box-body">
                            <div class="row">
                                <div class="col-md-8">
                                    ...
                                    <div class="col-md-6_">
                                        {!!Form::label("Select Brand(s) *")!!}
                                        <span class="pull-right"><a href="{{url('admin/products-brand')}}" target="_blank">Add Brands</a></span>
                                        <select name="brands[]" class="form-control select2_" id="example-getting-started" multiple="multiple")>
                                            @foreach($brandsData as $row)
                                                <option value="{{ $row->id }}" @if($singleData->id && in_array($row->id, $singleData->brands)) selected @endif>
                                                    {{ $row->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <em class="error-msg">{!!$errors->first('brands')!!}</em>
                                    </div>
                                    ...
                                    <div class="col-md-12_">
                                        <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                                            ...
                                            @if($singleData->image)
                                                <div class="row">
                                                    <div class="col-md-12">
                                                        <div class="image-close"><a href="{{url('admin/sales-person/'.$singleData->id.'/delete-image')}}"><i class="fa fa-close red-text"></i></a></div>
                                                        <img src="{{asset('storage/sales-persons/'.$singleData->image)}}" class="img-thumbnail" width="215px;">
                                                    </div>
                                                </div>
                                            @endif
                                            ...
                                            <table id="dataTable" class="table table-hover">
                                                ...
                                                @foreach ($allData as $row)
                                                    ...
                                                    <tr class="@if($row->status==0) disabledBg @endif">
                                                        ...
                                                        <td>
                                                            @if($row->image)
                                                                <img src="{{asset('storage/sales-persons/'.$row->image)}}" width="50px">
                                                            @else
                                                        ...
                                                        <td>{{ $row->phone_no }}</td>
                                                        <td>
                                                            @foreach ($row->haveBrands as $brandRow)
                                                                    <span class="label label-default">{{ $brandRow->brand->name }} </span> &nbsp
                                                            @endforeach
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td style="white-space: nowrap;">
                                                            <a href="{{ url('admin/sales-person/'.$row->id.'/edit')}}" class="btn btn-sm btn-warning"> <i class="fa fa-edit"> </i></a>
                                                            <a href="{{ url('admin/sales-person/'.$row->id.'/delete')}}" onclick="if(!confirm('Are you sure to delete this data?')){return false;}" class="btn btn-sm btn-danger"><i class="fa fa-trash-o"> </i></a>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                                </tbody>
