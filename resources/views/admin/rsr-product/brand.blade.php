...
@section('breadcrumb_title')
    <ol class="breadcrumb">
        ...
        <li class="text-capitalize active"> Brands</li>
    </ol>
@endsection
                ...
                <div class="row">
                    <div class="col-md-3">
                        ...
                        <div class="box-header">
                            <h3 class="box-title">
                                @if($singleData->id) Edit Brand ID: {{$singleData->id}} @else  Add New Brand @endif
                            </h3>
                        </div>
                        <div class="box-body">
                            ...
                            {{-- <div class="form-group {{ $errors->has('sales_person_id') ? 'has-error' : '' }}">
                                {!!Form::label("Sales Person ")!!}
                                {!!Form::select('sales_person_id', $salesPersonsData, null, ['class' => 'form-control select2', 'placeholder' => 'Select a sales person'])!!}
                                <em class="error-msg">{!!$errors->first('sales_person_id')!!}</em>
                            </div> --}}
                            @if($singleData->id)
                           ...
                           {{--  <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                                {!!Form::label("Upload Image[234*88]")!!}
                    ...
                    <div class="col-md-9">
                        <div class="box-header">
                            <h3 class="box-title">List of Product Brands</h3>
                            <small class="pull-right">
                                <a href="{{url('admin/products-brand')}}" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Add</a>
                            </small>
                        </div>
                        <div class="box-body">
                                ...
                                <table id="dataTable" class="table table-hover">
                                    ...
                                    @foreach ($allData as $row)
                                        ...
                                        <tr class="@if($row->status==0) disabledBg @endif">
                                            ...
                                            <td>
                                                @if($row->image)
                                                    <img src="{{asset('storage/brands/'.$row->image)}}" width="50px">
                                                @else
                                            ...
                                            <td>{!!$row->name!!}</td>
                                            <td>{!!$row->slug!!}</td>
                                            <td>{{ $row->sales_person_id ? $row->salesPersonSingle->title : '--' }}</td>
                                            <td>{!!$row->description!!}</td>
                                            <td>
                                                <a href="{{ url('admin/'.$module.'s-brand/'.$row->id.'/edit')}}" class="btn btn-sm btn-warning"> <i class="fa fa-edit"> </i></a>
                                                <a href="{{ url('admin/'.$module.'s-brand/'.$row->id.'/delete')}}" onclick="if(!confirm('Are you sure to delete this data?')){return false;}" class="btn btn-sm btn-danger"><i class="fa fa-trash-o"> </i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
